...
uint256 hashGenesisBlock("0x9aa65348dd0baf270c635b2b6d79e0bff2f856cd44b26fc5633daa8adf3adee6");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // PlanetDollar: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "PlanetDollar Signed Message:\n";
...
{
    // PlanetDollar: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 100 * COIN; //starting

    int64 nDays = nHeight/576; //720 blocks per day approx
    //int64 nDays = nHeight/2; //720 blocks per day approx

    if (nHeight == 1)
    {
        nSubsidy = 1200000000000 * COIN;  // premine to the tune of USD in circulation
    }
    else if (nHeight >1 && nHeight <= 200)  //launch at 200 with checkpoints, KGW kicks in at block 144 before that .. all zeroes
    {
    	nSubsidy = 0 * COIN;
    }
    else if (nHeight >200 && nHeight <800)  //real start of mining at 1600 anywhere between 1-2 days
    {
    	nSubsidy = 1 * COIN;
    }
    else
    {
    	...
    	nSubsidy += nDays*5*COIN; //Subsidy increases by 6 per day

		if ( (nHeight > 1073741824) || (nHeight < 0) ) //good for 4000 years or so.
		{
			nSubsidy = 0;  // at some point nHeight will wrap around i think.. at which point we are screwed anyways..
		}

		if (nSubsidy > 60000*COIN)
			nSubsidy = 60000*COIN;
...
int64 nTargetTimespan = 2.5 * 2 * 60; // Let's do every 2 blocks
static const int64 nTargetSpacing = 2.5 * 60; // PlanetDollar: 2.0 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // PlanetDollar: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // PlanetDollar: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x77776e80f30f560a6beeb5edbf48294f410d7632956b329168144e43a648c69f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        const char* pszTimestamp = "3/22/2014 Russia finalizes Crimea accession";
        ...
        block.nVersion = 1;
        block.nTime    = 1395472747; //final time
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 12693882;

        if (fTestNet)
        {
            block.nTime    = 1394785112;
            block.nBits    = 0x1e0ffff0;
            block.nNonce   = 10306804;
        }
        ...
        if (fTestNet)
        {
            assert(block.hashMerkleRoot == uint256("0x30b7e18c2493bce2439c320ec3ba3cfc641e50f7de6ade24d014c60c55660f4d"));
        }
        else
        {
            assert(block.hashMerkleRoot == uint256("0x30b7e18c2493bce2439c320ec3ba3cfc641e50f7de6ade24d014c60c55660f4d"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // PlanetDollar: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PlanetDollarMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PlanetDollarMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PlanetDollarMiner(CWallet *pwallet)
{
    printf("PlanetDollarMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("planetdollar-miner");
    ...
    try { loop {
        ...
        printf("Running PlanetDollarMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PlanetDollarMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PlanetDollarMiner, pwallet));
}
